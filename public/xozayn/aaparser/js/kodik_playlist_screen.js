$(document).ready(function() {
    if ( $("#kodik_player_ajax").attr("data-news_id") ) {
        ...
        if ( has_cache == 'no' ) {
            $.ajax({
                url: "/index.php?controller=ajax&mod=anime_grabber&module=kodik_playlist_ajax_screen",
                type: "POST",
                dataType: "html",
...
function del(news_id) {
    $.get(dle_root + "index.php?controller=ajax&mod=anime_grabber&module=kodik_watched", { 'news_id': news_id, 'action': 'delete_watched' }, function(data) {
		if ( data.status ) {
            $('.b-post__lastepisodeout').remove();
		}
	}, "json");
...
function voicerate_take (news_take) {
	$.ajax({
		url: "/index.php?controller=ajax&mod=anime_grabber&module=kodik_watched",
		type: "POST",
		dataType: "json",
...
function kodikMessageListener(message) {
	if ( message.data.key == 'kodik_player_current_episode' ) {
		...
		$.get(dle_root + "index.php?controller=ajax&mod=anime_grabber&module=kodik_watched", { 'news_id': news_id, 'kodik_data': message.data.value }, function(data) {
			if ( data.status ) {
		...
		$.get(dle_root + "index.php?controller=ajax&mod=anime_grabber&module=kodik_watched", { 'news_id': news_id, 'kodik_data': message.data.value, 'action': 'voicerate' }, function(data) {
			if ( data ) {
