$('.rooms-invite').on('click', function() {
    ...
    $.get(dle_root + "index.php?controller=ajax&mod=create_room", { news_id: news_id, iframe: news_iframe, title: news_title, poster: news_poster, shikimori_id: shikimori_id, mdl_id: mdl_id, user_hash: dle_login_hash }, function(data){
...
{	
    $.get(dle_root + "index.php?controller=ajax&mod=anime_grabber&module=push_subscribe", { post_id: post_id, subaction: action, user_hash: dle_login_hash }, function(data){
		if ( data.status ) {
			if ( action == 'subscribe' ) {
        		$('#push_subscribe').hide();
