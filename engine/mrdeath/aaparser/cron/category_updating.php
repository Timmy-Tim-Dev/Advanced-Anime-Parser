...
if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
	...
	$stage = $stage ?? 1;
	echo "=================================<br/>Начинаем обновление категории материала, старт (".date("Y-m-d H:i:s").")<br/>";
}
        ...
        if ( $will_check ) {
            ...
            if ( $news_row['id'] == $will_check['news_id'] ) {
                ...
                if ( $will_check['shikimori_id'] ) {
					...
					if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
						$time_update = microtime(true) - $time_update_start;
						echo "Этап ".$stage.": Получение данных shikimori id (".$will_check['shikimori_id'].") с API, прошло (".round($time_update,4)." секунд)<br/>";
						$stage++;
					}
                    if ( $shikimori['status'] ) {
                        ...
                        if ( !in_array($flipped_cats[$shikimori_status], $old_cats) ) {
							...
							if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
								$time_update = microtime(true) - $time_update_start;
								echo "Этап ".$stage.": Изменение статуса материала, прошло (".round($time_update,4)." секунд)<br/>";
								$stage++;
							}
                        }
                        if ( $will_be_updated === true ) {
							...
							if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
								$time_update = microtime(true) - $time_update_start;
								echo "Этап ".$stage.": Удаление материала news_id (".$news_row['id'].") с бд " . PREFIX . "_post_extras_cats, прошло (".round($time_update,4)." секунд)<br/>";
								$stage++;
							}
		                    $cat_ids = array ();
							...
							if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
								$time_update = microtime(true) - $time_update_start;
								echo "Этап ".$stage.": Добавление материала news_id (".$news_row['id'].") в бд " . PREFIX . "_post_extras_cats, прошло (".round($time_update,4)." секунд)<br/>";
								$stage++;
							}
							...
							if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
								$time_update = microtime(true) - $time_update_start;
								echo "Этап ".$stage.": Обновление материала news_id (".$news_row['id'].") в бд " . PREFIX . "_post, прошло (".round($time_update,4)." секунд)<br/>";
								$stage++;
							}
							...
							if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
								$time_update = microtime(true) - $time_update_start;
								echo "Этап ".$stage.": Обновление материала news_id (".$will_check['material_id'].") в бд " . PREFIX . "_anime_list, прошло (".round($time_update,4)." секунд)<br/>";
								$stage++;
							}
                        }
                    }
					...
					if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
						$time_update = microtime(true) - $time_update_start;
						echo "Этап ".$stage.": Получение данных mdl id (".$will_check['mdl_id'].") с API kodik, прошло (".round($time_update,4)." секунд)<br/>";
						$stage++;
					}
			...
			if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
				$time_update = microtime(true) - $time_update_start;
				echo "Закончили обновление категории материала, конец (".date("Y-m-d H:i:s")."), разница (".round($time_update,4)." секунд)<br/>=================================<br/>";
			}
	...
	elseif ( $xfields_data['kodik_duration_length'] ) $tags_array[] = $xfields_data['kodik_duration_length'];
	            ...
	            if ( $aaparser_config['categories'] AND $tags_array ) {
		            ...
		            if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
						$time_update = microtime(true) - $time_update_start;
						echo "Этап ".$stage.": Обработка категориев, прошло (".round($time_update,4)." секунд)<br/>";
						$stage++;
					}
					...
					if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
						$time_update = microtime(true) - $time_update_start;
						echo "Этап ".$stage.": Удаление материала news_id (".$news_row['id'].") в бд " . PREFIX . "_post_extras_cats, прошло (".round($time_update,4)." секунд)<br/>";
						$stage++;
					}
		            $cat_ids = array ();
			...
			if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
				$time_update = microtime(true) - $time_update_start;
				echo "Этап ".$stage.": Обновление материала news_id (".$news_row['material_id'].") в бд " . PREFIX . "_anime_list, прошло (".round($time_update,4)." секунд)<br/>";
				$stage++;
			}
			if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['category_material'] == 1 ) { 
				$time_update = microtime(true) - $time_update_start;
