...
*/
    ...
    foreach ( $kodik_updates as $anime_check ) {
		...
		$xf_shiki = "xfields LIKE '%".$aaparser_config['fields']['xf_shikimori_id']."|".$anime_check['shikimori_id']."||%'";
        ...
        if ( $aaparser_config['updates']['xf_quality'] && $quality && $anime_check['type'] != 'anime-serial' ) {
		...
		$descript = "";
		// Проверка анонас
		$anons_cats = explode(",", $checking_post['category']);
		
		if ($anime_check['material_data']['description'] !="" && in_array($aaparser_config['settings_anons']['cat_id'], $anons_cats) && $aaparser_config['settings_anons']['description_update'] == "1")
		{ 
