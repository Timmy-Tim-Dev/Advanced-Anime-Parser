...
*/
    ...
    if ( $kind == 'anime' ) {
        ...
        if ( !$material['material_id'] ) die('На данный момент нечего добавлять!');
    ...
    if ( isset( $aaparser_config['blacklist_shikimori'] ) && $xfields_data['shikimori_id'] ) {
	    if ( in_array( $xfields_data['shikimori_id'], $aaparser_config['blacklist_shikimori'] ) ) die($text_info_2.' '.$kodik_title.' входит в чёрный список аниме. Пропущено');
	}
