...
showRow('Домен Kodik', 'По умолчанию <b>bd.kodikres.com</b>, замените в случае переезда/замены домена Kodik.', showInput(['settings[kodik_site]', 'text', $aaparser_config['settings']['kodik_site']]));
showRow('Домен Kodik для плеера', 'По умолчанию <b>kodikplayer.com</b>, замените в случае переезда/замены домена Kodik для плеера.', showInput(['settings[kodik_api_player]', 'text', $aaparser_config['settings']['kodik_api_player']]));
showRow('Домен Kodik для запросов к api', 'По умолчанию <b>kodik-api.com</b>, замените в случае переезда/замены домена Kodik для запросов к api.', showInput(['settings[kodik_api_domain]', 'text', $aaparser_config['settings']['kodik_api_domain']]));
...
showRow('Использовать сторонний сервис для данных дорам?', 'Включив, модуль будет получать данные не напрямую а через API "https://api.allorigins.win/", рекомендуется пользоваться тем у кого не работает стандартный запрос по тем или иным причинам.', makeCheckBox('persons[personas_other_dorama_api]', $aaparser_config['persons']['personas_other_dorama_api']));
...
showRow('Домен Shikimori для запросов к api', 'По умолчанию <b>//shikimori.me/</b>, замените в случае переезда/замены домена Shikimori для запросов к api. Указываем в начале и со слешем в конце', showInput(['settings[shikimori_api_domain]', 'text', $aaparser_config['settings']['shikimori_api_domain']]));
showRow('Использовать старый метод API Shikimori?', 'В некоторых хостингах из за нового метода может не работать ручной поиск материала у аниме, но крон работает.<br>Но если у Вас не работает ручной поиск и Вам он нужен, то включите опцию, тогда будет по старому методу данные тянуть. <b>Обязательно необходимо указать выше API к Shikimori, если всё равно не работает, то проверьте другое API</b>', makeCheckBox('settings[alternate_shikimori]', $aaparser_config['settings']['alternate_shikimori']));
showRow('Парсить жанры и темы напрямую со страницы аниме на Shikimori?', 'Это экспериментальная опция, она была разработана в связи с тем, что не всегда в апи Shikimori присуствуют жанры аниме, хотя на самой странице данного аниме они указаны. Соответственно доп. поле с жанрами и категории созданные под жанры не проставляются. Активировав, вам будут доступны в настроках категорий новые жанры а так же множество новых тегов. Используйте тег {shikimori_genres} в разделе "Основные и доп поля", присвоив его доп. полю с жанрами<br/><b style="color:red">Занимает значительно больше времени для получения данных</b>', makeCheckBox('settings[parse_shikimori_genres]', $aaparser_config['settings']['parse_shikimori_genres']));
...
showRow('Парсить хронологию с Шикимори?', 'При отключении не будет использоваться тег {shikimori_chronology}. Отключение ускоряет работу парсинга<br/><b class="faq_find faq_id_2">Подробнее</b>', makeCheckBox('settings[parse_chronology]', $aaparser_config['settings']['parse_chronology']));
showRow('Сортировка хронологии с Шикимори', 'Работает только с включенным парсингом хронологии (Настройка выше)<br>Если включено, то сперва будут старые материалы, а затем новые. <br>При отключении, сперва будут новые материалы, а затем старые', makeCheckBox('settings[chronology_sort]', $aaparser_config['settings']['chronology_sort']));
