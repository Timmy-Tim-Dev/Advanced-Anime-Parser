...
showRow('Парсить постер с неофициального api MyAnimeList?', 'Это экспериментальная опция. Включив будет осуществлён парсинг <font style="color:red">трейлера и постера/обложки</font> аниме с помощью api jikan.moe. Shikimori отдаёт в api обложку маленького размера (225x310), данная опция позволит получать обложку большого размера (425x600)', makeCheckBox('settings[parse_jikan]', $aaparser_config['settings']['parse_jikan']));
