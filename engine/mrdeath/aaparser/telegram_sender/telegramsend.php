...
if ( isset($tlg_news_id) && isset($tlg_template) && isset($aaparser_config['push_notifications'][$tlg_template]) && isset($aaparser_config['push_notifications']['tg_bot_token']) && isset($aaparser_config['push_notifications']['tg_chanel']) ) {
    ...
    if (isset($row["id"])) {
        ...
        if (!empty($aaparser_config['push_notifications']['tg_only_ongoing']) &&
            $aaparser_config['push_notifications']['tg_only_ongoing'] != '-') {
            ...
            if (
                strpos($ongoing_value, mb_strtolower('Онгоинг', 'UTF-8')) === false &&
                strpos($ongoing_value, 'ongoing') === false
            ) {
                $db->query(
                    "DELETE FROM " . PREFIX . "_telegram_sender WHERE news_id='" . $tlg_news_id . "'"
                );
                if (isset($working_mode) && $working_mode == 'cron') {
                    echo 'News id: ' . $tlg_news_id . ' - пропуск по статусу';
                }
                $row = false;
            }
        }
