...
if ($parse_action == 'search') {
	...
	if ( $xfields_data['kodik_mydramalist_tags'] && isset($aaparser_config['settings']['translate_tags']) && $aaparser_config['settings']['translate_tags'] == 1 ) {
	    require ENGINE_DIR . '/mrdeath/aaparser/functions/GoogleTranslateForFree.php';
    ...
    if ($aaparser_config['grabbing']['translators']) {
		...
		$translation_key = array_search($aaparser_config['grabbing']['translators'], $translators);
		if ($translation_key === false) die('Значение не найдено в JSON');
		$translators_add = '&translation_id='.$translation_key;
	}
	...
	if ( $aaparser_config['grabbing_doram']['translators']) {
		...
		$translators = json_decode(file_get_contents($file_path), true);
		if (!is_array($translators)) die('Ошибка декодирования JSON');
		$translation_key = array_search($aaparser_config['grabbing_doram']['translators'], $translators);
		if ($translation_key === false) die('Значение не найдено в JSON');
		$translators_add = '&translation_id='.$translation_key;
	}
