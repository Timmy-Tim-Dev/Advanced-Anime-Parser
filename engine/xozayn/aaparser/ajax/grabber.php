...
require_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/telegram_sender/telegramsend_functions.php'));
...
$cron_log['time'] = $_TIME;
file_put_contents( ENGINE_DIR .'/xozayn/aaparser/data/cron.log', json_encode( $cron_log ));
...
} elseif ( $action == 'update' && $kind == 'anime' ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/update_anime.php'));
elseif ( $action == 'update' && $kind == 'dorama' ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/update_dorama.php'));
elseif ( $action == 'add') {
	if (isset($aaparser_config['settings']['grab_on'])) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/add_material.php'));
	else echo "Функционал граббинга отключен, пожалуйста включите это в настройках модуля!";
}
elseif ( $action == 'category_updating' ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/category_updating.php'));
elseif ( $action == 'xfields_updating' ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/xfields_updating.php'));
elseif ( $action == 'other' ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/other_actions.php'));
elseif ( $action == 'anons_shiki' ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/anons_shiki.php'));
elseif ( $action == 'anons_clean' ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/anons_clean.php'));
elseif ( $action == 'update_franchise' ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/cron/update_franchise.php'));
