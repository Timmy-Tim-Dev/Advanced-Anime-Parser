...
if (isset($aaparser_config['settings']['first_time']) && $aaparser_config['settings']['first_time'] == 1) {
	echo <<<HTML
	<div id="aapfirsttimesettings" hidden>
...
HTML;
if ($php_version >= 74 && file_exists(ENGINE_DIR.'/xozayn/aaparser/google_indexing/indexing.php')) echo "ShowOrHideGindexing();";
echo <<<HTML
	$(function() {
		...
		function aaparser_save_option() {
		    ...
		    if ( document.querySelector('[name="settings[kodik_api_key]"]').value == "" ) {
		        DLEPush.error('Поле с вашим api токеном от базы Kodik не может быть пустым. Заполните его');
				HideLoading("");
		    ...
		    if ( document.querySelector('[name="xfields[alt_name]"]').value == "" ) {
		        DLEPush.error('Поле с ЧПУ URL статьи не может быть пустым. Заполните его');
				HideLoading("");
				$('body button[type=submit]').css("pointer-events","unset").css("background","#009688").html('<i class="fa fa-floppy-o position-left"></i> Сохранить');
				return false;
		    }
			...
			$.post('/index.php?controller=ajax&mod=anime_grabber&module=aaparser_save', {data_form: data_form, action: 'options', user_hash: '{$dle_login_hash}'}, function(data) {
				...
				if (!data.success) {
					...
					DLEPush.info('Настройки модуля были успешно сохранены');
