...
showRow('Порядок постера материала', 'Если включен, то сперва будет проверяться наличие постера на Shikimori (Аниме) или MDL (Дорама), если есть, то их вставлять, если нет, то вставит с Kinopoisk. Если выключен, то сразу с Kinopoisk', makeCheckBox('grabbing[poster_source]', $aaparser_config['grabbing']['poster_source']));
...
showRow('Добавлять только аниме с LGBT сценами?', 'Если включено то на сайт будут добавляться только аниме с LGBT сценами. <b>Обязательно необходимо включить "Добавлять аниме с LGBT сценами?" *Условие будет пропущено, если данного аниме нет в базе Kodik!</b>', makeCheckBox('grabbing[if_lgbt_find]', $aaparser_config['grabbing']['if_lgbt_find']));
...
showRow('Добавлять только дорамы с LGBT сценами?', 'Если включено то на сайт будут добавляться только дорамы с LGBT сценами. <b>Обязательно необходимо включить "Добавлять дорамы с LGBT сценами?" *Условие будет пропущено, если данного аниме нет в базе Kodik!</b>', makeCheckBox('grabbing_doram[if_lgbt_find]', $aaparser_config['grabbing_doram']['if_lgbt_find']));
