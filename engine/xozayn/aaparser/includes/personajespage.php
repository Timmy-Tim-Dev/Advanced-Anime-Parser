...
$personajes_cache_size = convert_bytes(dir_size(ENGINE_DIR."/xozayn/aaparser/cache/personas_characters/"));
$personajes_dorama_cache_size = convert_bytes(dir_size(ENGINE_DIR."/xozayn/aaparser/cache/dorama_persons_page/"));
$personajes_page_cache_size = convert_bytes(dir_size(ENGINE_DIR."/xozayn/aaparser/cache/personas_characters_page/"));
...
showRow('Выводить главных персонажей?', 'Включив, модуль будет выводить главных персонажей, будет активирован тег {kodik_main_characters}', makeCheckBox('persons[main_characters]', $aaparser_config['persons']['main_characters']));
showRow('Лимит главных персонажей', 'Вы можете задать лимит количества главных персонажей, для отключеня лимита вставьте 0', showInput(['persons[main_characters_limit]', 'number', $aaparser_config['persons']['main_characters_limit']]));
showRow('Выводить второстепенных персонажей?', 'Включив, модуль будет выводить второстепенных персонажей, будет активирован тег {kodik_sub_characters}', makeCheckBox('persons[characters]', $aaparser_config['persons']['characters']));
showRow('Лимит второстепенных персонажей', 'Вы можете задать лимит количества второстепенных персонажей, для отключеня лимита вставьте 0', showInput(['persons[characters_limit]', 'number', $aaparser_config['persons']['characters_limit']]));
showRow('Выводить персон (актёры, режиссёры  и другие участники аниме)?', 'Включив, модуль будет выводить персон, будет активирован тег {kodik_persons}', makeCheckBox('persons[persons]', $aaparser_config['persons']['persons']));
showRow('Лимит персон', 'Вы можете задать лимит количества персон, для отключеня лимита вставьте 0', showInput(['persons[persons_limit]', 'number', $aaparser_config['persons']['persons_limit']]));
showRow('Кэшировать данные персонажей и авторов?', 'Включив, модуль будет кэшировать полученные данные, заметно ускоряет обработку страницы<br/><b>Настоятельно рекомендиуем использовать кэширование, значительно ускоряет</b>', makeCheckBox('persons[personas_cache]', $aaparser_config['persons']['personas_cache']));
...
showRow('Постер при отсутствий изображения', 'Укажите путь до картинки заглушки для отсутствующих постеров. <br/><b>Для корректной работы, укажите прямую ссылку до картинки</b><br/>Пример: <i>/templates/Default/dleimages/no_image.jpg</i>', showInput(['persons[default_image]', 'text', $aaparser_config['persons']['default_image']]));
showRow('Включить обработку страниц персонажей и авторов аниме?', 'Включить вывод обработки страниц персонажей и авторов аниме взятых из Shikimori', makeCheckBox('persons[persons_page]', $aaparser_config['persons']['persons_page']));
showRow('Кэшировать данные страниц персонажей и авторов?', 'Включив, модуль будет кэшировать полученные данные, заметно ускоряет обработку страницы<br/><b>Настоятельно рекомендуем использовать кэширование!</b>', makeCheckBox('persons[persons_page_cache]', $aaparser_config['persons']['persons_page_cache']));
...
showRow('Метатег "title" для страницы персонажей и авторов', 'Если оставите пустым, то будет выведен стандартный от DLE. <br/>Список доступных тегов: '. $avaiable_tags_chars, showInput(['persons[metatitle]', 'text', $aaparser_config['persons']['metatitle']]));
showRow('Метатег "description" для страницы персонажей и авторов', 'Если оставите пустым, то будет выведен стандартный от DLE. <br/>Список доступных тегов: '. $avaiable_tags_chars, showInput(['persons[metadescr]', 'text', $aaparser_config['persons']['metadescr']]));
showRow('Метатег "keywords" для страницы персонажей и авторов', 'Если оставите пустым, то будет выведен стандартный от DLE. <br/>Список доступных тегов: '. $avaiable_tags_chars, showInput(['persons[metakeyw]', 'text', $aaparser_config['persons']['metakeyw']]));
showRow('Включить страницы персонажей и авторов в карту сайта?', 'Если включено, то страницы персонажей и авторов будут выводиться в карту сайта и будут доступны по адресу <a href="'.$config["http_home_url"].'people.xml" target="_blank">people.xml</a> и <a href="'.$config["http_home_url"].'characters.xml" target="_blank">characters.xml</a><br/><b>Важно!</b> При отключении, созданные страницы с карты сайта не пропадут, если необходимо удалить их тоже, то необходимо удалить файлы <i>characters.dat</i> и <i>people.dat</i> в папке <i>data</i> и создать новую <i>карту сайта</i>', makeCheckBox('persons[persons_sitemap]', $aaparser_config['persons']['persons_sitemap']));
echo <<<HTML
		</table>
...
Ссылка ЧПУ "/people/{char_id}[/]" Реальная ссылка "/index.php?do=characters&type=people&id={char_id}"</textarea>
			...
			<textarea style="width:100%;height:25px;" disabled>RewriteRule ^characters(\d*?).xml$ uploads/characters$1.xml [L]</textarea>
		</div>
	</div>
</div>
