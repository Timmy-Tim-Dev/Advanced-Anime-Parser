...
require_once ENGINE_DIR . '/xozayn/aaparser/functions/tags_list.php';

$xfield_list = DLEXFields::GetFieldsNames();
// print_r($xfield_list);
...
for ($i = 0; $i < count($xfield_list); $i++) {
    $xfields_all_list[$xfield_list[$i]] = DLEXFields::GETField($xfield_list[$i])["description"];
	if ( DLEXFields::GETField($xfield_list[$i])["type"] == "text" OR DLEXFields::GETField($xfield_list[$i])["type"] == "textarea" OR DLEXFields::GETField($xfield_list[$i])["type"] == "htmljs" ) {
	    $main_fields[$xfield_list[$i]] = DLEXFields::GETField($xfield_list[$i])["description"];
	    $xfields_list[$xfield_list[$i]] = DLEXFields::GETField($xfield_list[$i])["description"];
	}
	elseif ( DLEXFields::GETField($xfield_list[$i])["type"] == "image" ) $xfield_image[$xfield_list[$i]] = DLEXFields::GETField($xfield_list[$i])["description"];
	elseif ( DLEXFields::GETField($xfield_list[$i])["type"] == "imagegalery" ) $xfield_gallery[$xfield_list[$i]] = DLEXFields::GETField($xfield_list[$i])["description"];
	elseif ( DLEXFields::GETField($xfield_list[$i])["type"] == "yesorno" ) $xfield_yesorno[$xfield_list[$i]] = DLEXFields::GETField($xfield_list[$i])["description"];
	elseif ( DLEXFields::GETField($xfield_list[$i])["type"] == "select" ) $xfield_select[$xfield_list[$i]] = DLEXFields::GETField($xfield_list[$i])["description"];
}
...
if ($aaparser_config['settings']['working_mode'] == 1) {
	if (!file_exists(ENGINE_DIR."/xozayn/aaparser/data/translators_name_dorama.json") || filectime(ENGINE_DIR."/xozayn/aaparser/data/translators_name_dorama.json") < $c_time) {
		//dorama
		if ($aaparser_config['settings']['kodik_api_domain'] != '') $cont = file_get_contents($protocol . $aaparser_config['settings']['kodik_api_domain']."/translations/v2?token=".$aaparser_config['settings']['kodik_api_key']."&types=foreign-movie,foreign-serial");
		else $cont = file_get_contents($protocol . "kodik-api.com/translations/v2?token=".$aaparser_config['settings']['kodik_api_key']."&types=foreign-movie,foreign-serial");
		...
		file_put_contents(ENGINE_DIR.'/xozayn/aaparser/data/translators_name_dorama.json', json_encode($translators_name, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ));
		file_put_contents(ENGINE_DIR.'/xozayn/aaparser/data/translators_dorama.json', json_encode($translators, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ));
	...
	if (!file_exists(ENGINE_DIR."/xozayn/aaparser/data/translators_name.json") || !file_exists(ENGINE_DIR."/xozayn/aaparser/data/translators_name_dorama.json") || filectime(ENGINE_DIR."/xozayn/aaparser/data/translators_name.json") < $c_time || filectime(ENGINE_DIR."/xozayn/aaparser/data/translators_name_dorama.json") < $c_time) {
		...
		unset ($translators_name, $translators, $cont);
		//anime
		if ($aaparser_config['settings']['kodik_api_domain'] != '') $cont = file_get_contents($protocol . $aaparser_config['settings']['kodik_api_domain']."/translations/v2?token=".$aaparser_config['settings']['kodik_api_key']."&types=anime,anime-serial");
		else $cont = file_get_contents($protocol . "kodik-api.com/translations/v2?token=".$aaparser_config['settings']['kodik_api_key']."&types=anime,anime-serial");
		$cont = json_decode($cont, true);
		$translators_name = $translators = [];
		if (isset($cont['results'])) {
			foreach ($cont['results'] as $result) {
		...
		file_put_contents(ENGINE_DIR.'/xozayn/aaparser/data/translators_name.json', json_encode($translators_name, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ));
		file_put_contents(ENGINE_DIR.'/xozayn/aaparser/data/translators.json', json_encode($translators, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ));
		unset ($translators_name, $translators, $cont);
	}
} else {
...
if (!file_exists(ENGINE_DIR."/xozayn/aaparser/data/countries_name.json") || filectime(ENGINE_DIR."/xozayn/aaparser/data/countries_name.json") < $c_time) {
	if ($aaparser_config['settings']['kodik_api_domain'] != '') $cont = file_get_contents($protocol . $aaparser_config['settings']['kodik_api_domain']."/countries?token=".$aaparser_config['settings']['kodik_api_key']);
	else $cont = file_get_contents($protocol . "kodik-api.com/countries?token=".$aaparser_config['settings']['kodik_api_key']);
	$cont = json_decode($cont, true);
	$countries_name = [];
	if (isset($cont['results'])) {
		foreach ($cont['results'] as $result) {
	...
	file_put_contents(ENGINE_DIR.'/xozayn/aaparser/data/countries_name.json', json_encode($countries_name, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ));
	unset ($countries_name, $cont);
} 

if ( file_exists(ENGINE_DIR.'/xozayn/aaparser/data/countries_name.json') ) {
    $countries_array = file_get_contents(ENGINE_DIR.'/xozayn/aaparser/data/countries_name.json');
    $countries_array = json_decode($countries_array, true);
	$countries_array = preg_replace('/"([^"]*?)"(?=[^"]*?"|$)/', '\"$1\"', $countries_array);
}
else $countries_array = [];

if ( file_exists(ENGINE_DIR.'/xozayn/aaparser/data/translators_name.json') ) {
    $translator_array = file_get_contents(ENGINE_DIR.'/xozayn/aaparser/data/translators_name.json');
    $translator_array = json_decode($translator_array, true);
	$translator_array = preg_replace('/"([^"]*?)"(?=[^"]*?"|$)/', '\"$1\"', $translator_array);
}
else $translator_array = [];

if ( file_exists(ENGINE_DIR.'/xozayn/aaparser/data/translators.json') ) {
    $translators_array = file_get_contents(ENGINE_DIR.'/xozayn/aaparser/data/translators.json');
...
if ( file_exists(ENGINE_DIR.'/xozayn/aaparser/data/translators_name_dorama.json') ) {
    $translator_array_dorama = file_get_contents(ENGINE_DIR.'/xozayn/aaparser/data/translators_name_dorama.json');
...
if ( file_exists(ENGINE_DIR.'/xozayn/aaparser/data/translators_dorama.json') ) {
    $translators_array_dorama = file_get_contents(ENGINE_DIR.'/xozayn/aaparser/data/translators_dorama.json');
...
if (!file_exists(ENGINE_DIR."/xozayn/aaparser/data/mydramalist.json") || filectime(ENGINE_DIR."/xozayn/aaparser/data/mydramalist.json") < $c_time || file_get_contents(ENGINE_DIR."/xozayn/aaparser/data/mydramalist.json") == '') {
	if ($aaparser_config['settings']['kodik_api_domain'] != '') $cont = file_get_contents($protocol . $aaparser_config['settings']['kodik_api_domain']."/genres?token=".$aaparser_config['settings']['kodik_api_key']."&genres_type=mydramalist");
	else $cont = file_get_contents($protocol . "kodik-api.com/genres?token=".$aaparser_config['settings']['kodik_api_key']."&genres_type=mydramalist");
...
if (!file_exists(ENGINE_DIR."/xozayn/aaparser/data/shikimori.json") || filectime(ENGINE_DIR."/xozayn/aaparser/data/shikimori.json") < $c_time || file_get_contents(ENGINE_DIR."/xozayn/aaparser/data/shikimori.json") == '') {
	if ($aaparser_config['settings']['kodik_api_domain'] != '') $cont = file_get_contents($protocol . $aaparser_config['settings']['kodik_api_domain']."/genres?token=".$aaparser_config['settings']['kodik_api_key']."&genres_type=shikimori");
	else $cont = file_get_contents($protocol . "kodik-api.com/genres?token=".$aaparser_config['settings']['kodik_api_key']."&genres_type=shikimori");
...
if (!file_exists(ENGINE_DIR."/xozayn/aaparser/data/kinopoisk.json") || filectime(ENGINE_DIR."/xozayn/aaparser/data/kinopoisk.json") < $c_time || file_get_contents(ENGINE_DIR."/xozayn/aaparser/data/kinopoisk.json") == '') {
	if ($aaparser_config['settings']['kodik_api_domain'] != '') $cont = file_get_contents($protocol . $aaparser_config['settings']['kodik_api_domain']."/genres?token=".$aaparser_config['settings']['kodik_api_key']."&genres_type=kinopoisk");
	else $cont = file_get_contents($protocol . "kodik-api.com/genres?token=".$aaparser_config['settings']['kodik_api_key']."&genres_type=kinopoisk");
...
$mydramalist = file_get_contents(ROOT_DIR."/engine/xozayn/aaparser/data/mydramalist.json");
...
$shikimori = file_get_contents(ROOT_DIR."/engine/xozayn/aaparser/data/shikimori.json");
...
$kinopoisk = file_get_contents(ROOT_DIR."/engine/xozayn/aaparser/data/kinopoisk.json");
