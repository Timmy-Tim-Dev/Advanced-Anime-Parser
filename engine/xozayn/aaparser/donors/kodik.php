...
if ($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['donors'] == 1) { 
	$debugger_table_row .= tableRowCreate("(kodik.php) Начинаем инициализацию донора kodik.php", round(microtime(true) - $time_update_start, 4));
}
...
$kodik_apikey = isset($aaparser_config['settings']['kodik_api_key']) ? $aaparser_config['settings']['kodik_api_key'] : die("Нету ключа API домена");
...
if ($parse_action == 'search') {
	...
	if ( $kodik['results'] ) {
		...
		if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['donors'] == 1 ) { 
			$debugger_table_row .= tableRowCreate("(kodik.php) Разобрали полученный запрос", round(microtime(true) - $time_update_start,4));
		}
	}
	...
	if ( $xfields_data['kodik_mydramalist_tags'] && isset($aaparser_config['settings']['translate_tags']) && $aaparser_config['settings']['translate_tags'] == 1 ) {
	    require ENGINE_DIR . '/xozayn/aaparser/functions/GoogleTranslateForFree.php';
	...
	if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['donors'] == 1 ) { 
		$debugger_table_row .= tableRowCreate("(kodik.php) Распределение по тегам", round(microtime(true) - $time_update_start,4));
	}
	...
	if (!empty($aaparser_config['grabbing']['translators']) || !empty($aaparser_config['grabbing']['not_translators'])) {
		$file_path = ENGINE_DIR . '/xozayn/aaparser/data/translators.json';
	...
	if (!empty($aaparser_config['grabbing']['countries'])) {
		$file_path = ENGINE_DIR . '/xozayn/aaparser/data/countries_name.json';
		if (!file_exists($file_path)) die('У Вас не загружены озвучки');
    ...
    if ( !$grab['results'] ) {
		...
		file_put_contents( ENGINE_DIR .'/xozayn/aaparser/data/kodik.log', json_encode( $kodik_log ));
		if ($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['donors'] == 1) { 
	...
	if (!empty($aaparser_config['grabbing_doram']['translators']) || !empty($aaparser_config['grabbing_doram']['not_translators'])) {
		$file_path = ENGINE_DIR . '/xozayn/aaparser/data/translators_dorama.json';
		if (!file_exists($file_path)) die('У Вас не загружены озвучки');
		$translators = json_decode(file_get_contents($file_path), true);
		if (!is_array($translators)) die('Ошибка декодирования JSON');
	}
        ...
        foreach ($grab['results'] as $result) {
            ...
            if ( $where ) {
				...
				if ( $cheking['material_id'] > 0 ) {
					...
					$grab_info_old .= $grab_num_old.'. Дорама '.$result['title'].' есть в базе модуля. Пропущено<br>';
					$grab_num_old++;
				} else {
					...
					$grab_info_new .= $grab_num_new.'. Дорама '.$result['title'].' добавлено в базу модуля<br>';
					$grab_num_new++;
				}
			} else continue;
        }
