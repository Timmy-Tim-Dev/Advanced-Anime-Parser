...
\$aaparser_config = array (
  ...
  array (
	'first_time' => 1,
    'kodik_api_key' => '',
    'kodik_site' => 'https://bd.kodikres.com',
    'kodik_api_domain' => 'https://kodik-api.com/',
    'kodik_api_player' => 'https://kodikplayer.com',
...
HTML;
  	...
  	$fp = fopen(ENGINE_DIR.'/xozayn/aaparser/data/config.php', "w+");
	@chmod(ENGINE_DIR.'/xozayn/aaparser/data/config.php', 0777);
  	fwrite($fp, $text);
  	fclose($fp);
  	unset($text);
...
if (!file_exists(ENGINE_DIR.'/xozayn/aaparser/data/kodik.log')) {
  	$fp = fopen(ENGINE_DIR.'/xozayn/aaparser/data/kodik.log', "w+");
	@chmod(ENGINE_DIR.'/xozayn/aaparser/data/kodik.log', 0777);
  	fwrite($fp, "");
  	fclose($fp);
}

if (!file_exists(ENGINE_DIR.'/xozayn/aaparser/data/cron.log')) {
  	$fp = fopen(ENGINE_DIR.'/xozayn/aaparser/data/cron.log', "w+");
	@chmod(ENGINE_DIR.'/xozayn/aaparser/data/cron.log', 0777);
  	fwrite($fp, "");
  	fclose($fp);
}

if (!file_exists(ENGINE_DIR.'/xozayn/aaparser/data/updates_history.json') && $aaparser_config['updates_block']['enable_history'] == 1 ) {
  	$fp = fopen(ENGINE_DIR.'/xozayn/aaparser/data/updates_history.json', "w+");
	@chmod(ENGINE_DIR.'/xozayn/aaparser/data/updates_history.json', 0777);
  	fwrite($fp, "[]");
  	fclose($fp);
}

if (file_exists(ENGINE_DIR.'/xozayn/aaparser/data/version.php')) {
	$log_module_version = file_get_contents(ENGINE_DIR.'/xozayn/aaparser/data/version.php');
} elseif (!file_exists(ENGINE_DIR.'/xozayn/aaparser/data/version.php') && file_exists(ENGINE_DIR.'/xozayn/aaparser/data/version.log')) {
    $log_module_version_old = file_get_contents(ENGINE_DIR.'/xozayn/aaparser/data/version.log');
	$fp = fopen(ENGINE_DIR.'/xozayn/aaparser/data/version.php', "w+");
	@chmod(ENGINE_DIR.'/xozayn/aaparser/data/version.php', 0777);
...
if ( file_exists(ENGINE_DIR.'/xozayn/aaparser/google_indexing/indexing.php') ) {
	if ( file_exists(ENGINE_DIR.'/xozayn/aaparser/google_indexing/data/indexing.json') ) {
		...
		$mod_settings['logs'][] = '';
		$fp = fopen(ENGINE_DIR.'/xozayn/aaparser/google_indexing/data/indexing.json', "w+");
		@chmod(ENGINE_DIR.'/xozayn/aaparser/google_indexing/data/indexing.json', 0777);
    ...
    $acdir = opendir(ENGINE_DIR.'/xozayn/aaparser/google_indexing/accounts/');
    $aclist = [];
...
require_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/functions/admin.php'));
...
if ($aaparser_config['settings']['cron_key'] == "mrd3ath" || $aaparser_config['settings']['cron_key'] == "y0urcr0nk3y") {
    ...
    file_put_contents(ENGINE_DIR.'/xozayn/aaparser/data/config.php', $config_content);
}
...
<style>
HTML;
require_once (DLEPlugins::Check(ENGINE_DIR .'/xozayn/aaparser/admin/styles.css'));
echo <<<HTML
</style>

<script>
HTML;
require_once (DLEPlugins::Check(ENGINE_DIR .'/xozayn/aaparser/admin/scripts.js'));
echo <<<HTML
</script>

HTML;
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/navbar.php')); //Навигация

// Проверка доступа к Data и его элементам
$data_items = glob( ENGINE_DIR . "/xozayn/aaparser/data/*");
...
$perm_f = ENGINE_DIR.'/xozayn/aaparser/data/';
...
HTML;
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/mainpage.php')); //Основные настройки
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/grabpage.php')); //Настройки граббинга
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/updatenewspage.php')); //Обновление новостей
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/updatespage.php')); //Поднятие новостей
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/xfieldspage.php')); //Основные и доп поля
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/catspage.php')); //Категории
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/imagespage.php')); //Изображения
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/integrationpage.php')); //Интеграция
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/cronpage.php')); //Настройки планировщика
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/anonspage.php')); //Настройки Анонса
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/faqpage.php')); //Часто задаваемые вопросы
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/modulespage.php')); //Модули
echo <<<HTML
...
HTML;
include_once (DLEPlugins::Check(ENGINE_DIR.'/xozayn/aaparser/includes/scripts.php')); // Скрипты 
echo <<<HTML
...
elseif ( $action == 'dbupgrade' ) {
		...
		$versions = array_filter($versions, function ($verik) use ($log_module_version) {
			return version_compare($verik, $log_module_version, '>');
		});
		$files = glob( ENGINE_DIR . "/xozayn/aaparser/includes/upgrade/*");
