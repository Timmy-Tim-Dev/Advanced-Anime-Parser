...
if ($action == 'options') {
	...
	$handler = fopen(ENGINE_DIR . '/xozayn/aaparser/data/config.php', "w");
...
if ($php_version >= 74 && file_exists(ENGINE_DIR.'/xozayn/aaparser/google_indexing/indexing.php') && $aaparser_config['push_notifications']['google_indexing'] = "1") {
	...
	if ( file_exists(ENGINE_DIR.'/xozayn/aaparser/google_indexing/data/indexing.json') ) {
		...
		file_put_contents(ENGINE_DIR.'/xozayn/aaparser/google_indexing/data/indexing.json', json_encode($mod_settings, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ));
	} else {
		$mod_settings = [];
