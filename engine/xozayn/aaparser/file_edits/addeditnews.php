...
<script type="text/javascript">
    ...
    function parser_search(){
            ...
            ShowLoading('');
            $.ajax({
                url: "/index.php?controller=ajax&mod=anime_grabber&module=anime_parser",
                ...
                success: function(data) {
					if ( data.status == "results" ) {
                        ...
                        $.each(data.result, function(key, item) {
							...
							if ( item.kodik_exists ) results += "<td><span class=\"pntr\" data-balloon-length=\"medium\" aria-label=\"Есть в базе Kodik\" data-balloon-pos=\"up\"><b><img src=\"<?php echo $aaparser_config['settings']['kodik_site'] ?>packages/images/favicon.png\"></b></span></td>";
							else results += "<td> </td>";
							if (item.find_id == 'est') {
    ...
    function parser_paste(data){
        ...
        if( data.short_story ) {
            ...
            if ( !shortstory ) {
                <?php echo ($short_st ?? '') !== '' ? $short_st : '('; ?>data.short_story<?php echo ");\n"; ?>
            }
        }
        if( data.full_story ) {
            var fullstory = $("#full_story").val();
            if ( !fullstory ) {
                <?php echo ($full_st ?? '') !== '' ? $full_st : '(';; ?>data.full_story<?php echo ");\n"; ?>
            }
        }
