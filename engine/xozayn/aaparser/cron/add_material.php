...
if ( $kind == 'anime' ) {
	...
	include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/donors/shikimori.php'));
	include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/donors/kodik.php'));
	if ( $aaparser_config['settings']['parse_wa'] == 1 ) include_once (DLEPlugins::Check(ENGINE_DIR . '/xozayn/aaparser/donors/world_art.php'));
	if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['add_material'] == 1 ) {
...
else $db->query( "INSERT INTO " . PREFIX . "_post_extras (news_id, allow_rate, disable_index, user_id, disable_search, allow_rss, allow_rss_dzen) VALUES ('{$id}', '{$allow_rating}', '{$dissalow_index}', '{$author_id}', '{$dissalow_search}', '{$allow_rss}', '{$allow_zen}')" );
if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['add_material'] == 1 ) {
...
$newpostedxfields = DLEXFields::xfieldsdataload($xfields_list);
// print_r(newpostedxfields);
// die();
...
if ( $aaparser_config['integration']['ksep'] == 1 && file_exists(ENGINE_DIR.'/xozayn/ksep/modules/aap.php') ) {
    ...
    require_once ENGINE_DIR.'/xozayn/ksep/modules/aap.php';
	if($aaparser_config['debugger']['enable'] == 1 && $aaparser_config['debugger']['add_material'] == 1 ) { 
